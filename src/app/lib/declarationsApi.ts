...
function declarationRowToTableRowData(row: any, invoices: InvoiceRow[]): TableRowData {
  return migrateRecord({
    ...
    currencyRateDate: row.currency_rate_date,
    stage: row.stage,
...
function tableRowDataToDeclarationRow(data: Omit<TableRowData, 'id'> | TableRowData) {
  return {
    ...
    currency_rate_date: data.currencyRateDate ?? '',
...
export async function updateDeclaration(id: string, updates: Partial<TableRowData>): Promise<void> {
  ...
  const keyMap: Record<string, string> = {
    ...
    noOfParcels: 'no_of_parcels', freightAndCosts: 'freight_and_costs', currencyRate: 'currency_rate', currencyRateDate: 'currency_rate_date', stage: 'stage', sentDate: 'sent_date', validated: 'validated',
